...
它声称有极佳的性能，占用空间更小，且提供其他日志系统缺失但很有用的特性。 其一大特色是，在 logback-classic 中本地(native)实现了
SLF4J API（也表示依赖 slf4j-api）

## 添加依赖

实际开发中我们直接引入`spring-boot-starter-web`依赖即可，因为`spring-boot-starter-web`包含了`spring-boot-starter`。 而`spring-boot-starter`包含了`spring-boot-starter-logging`，所以我们只需要引入 web 组件即可。
...
<configuration debug="false">
    <!--定义应用的名称-->
    <property name="APP_NAME" value="solution-sales-service"/>
    ...
    <appender name="FILE" class="ch.qos.logback.core.rolling.RollingFileAppender">
        ...
        <encoder class="ch.qos.logback.classic.encoder.PatternLayoutEncoder">
            <!--格式化输出：%d表示日期，%thread表示线程名，%-5level：级别从左显示5个字符宽度%msg：日志消息，%n是换行符-->
            <pattern>%contextName |- %d{yyyy-MM-dd HH:mm:ss.SSS} [%thread] %-5level %logger{35}.%method:%line - %msg
                %n
            </pattern>
            <!-- 或者使用默认配置 -->
